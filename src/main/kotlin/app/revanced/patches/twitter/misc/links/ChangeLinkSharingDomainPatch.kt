...
object ChangeLinkSharingDomainPatch : BytecodePatch(
    ...
    override fun execute(context: BytecodeContext) {
        ...
        LinkResourceGetterFingerprint.result?.mutableMethod?.apply {
            ...
            // Format the link with the new domain name call address. (2 instructions above the const-string)
            val formatLinkCallIndex = constWithParameterName - 2

            val formatLinkCall = getInstruction<BuilderInstruction35c>(formatLinkCallIndex)
            if (formatLinkCall.registerCount != 3) throw LinkResourceGetterFingerprint.exception
