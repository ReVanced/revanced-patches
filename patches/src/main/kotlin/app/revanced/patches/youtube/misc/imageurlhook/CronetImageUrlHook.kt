...
val cronetImageUrlHookPatch = bytecodePatch(
    ...
    execute { context ->
        fun Fingerprint.alsoResolve(match: Match) =
            also { this.match(context, match.classDef) }.matchOrThrow()

        loadImageUrlMethod = messageDigestImageUrlFingerprint
            .alsoResolve(messageDigestImageUrlParentMatch).mutableMethod

        loadImageSuccessCallbackMethod = onSucceededFingerprint
            .alsoResolve(onResponseStartedMatch).mutableMethod
